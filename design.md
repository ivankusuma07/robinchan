...
- **A full-bleed hero backdrop** — two blurred, slowly drifting accent-green and companion-pink blobs behind the hero content only (`animate-orb-drift`). This is the one place on the whole product a decorative gradient is allowed, and the one place `companion-pink` is allowed outside strictly character-adjacent chrome (§2) — it's brand-mood, not a UI signal, and it fades to flat black before the next section so it reads as "hero backdrop," not a site-wide tint.
...
- **Hover feedback everywhere a card or step is a real destination or a real idea** — feature cards lift and pick up their tone color on hover; capital-flow steps highlight on hover. None of this touches `/market` or the dashboard's data-driven cards — those still follow §6/§8 exactly as written.
