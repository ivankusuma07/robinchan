...
import type { Program as OglProgram } from 'ogl';

/**
 * WebGL fiber-field background, from React Bits
 * (https://reactbits.dev — "GhostFibers"), ported to TypeScript and recolored
 * to the site's own tokens. Used once, as the Home hero's backdrop
 * (design.md §10) — replaces the earlier pair of blurred CSS orbs with the
 * same job: a slow, low-key accent-green/companion-pink drift behind the
 * hero, nowhere else.
 *
 * Left mostly as upstream wrote it (the shader and the render loop are
 * exactly reactbits.dev's own — no reason to rewrite working WebGL code).
 * What changed for this project:
 * - Ported to TypeScript against `ogl`'s own shipped types.
 * - Dropped the sibling `.css` file — this codebase styles everything with
 *   Tailwind utility classes, no per-component stylesheets elsewhere.
 * - Wrapped renderer setup in try/catch: this is pure decoration, so a
 *   WebGL2 failure (unlike the Live2D stage, where the character is the
 *   actual product) should just mean no background, never an error state.
 */
...
const setColor = (uniform: { value: Float32Array }, hex: string): void => {
...
/**
 * Defaults are this project's own — accent green + companion-pink, and
 * noticeably calmer than reactbits.dev's out-of-the-box preset (slower,
 * fewer layers, lower brightness/glow). The example values are tuned for a
 * standalone demo; a background behind hero copy needs to stay in the
 * peripheral vision, not compete with it (design.md §8's restraint rule
 * applies here too, even though this section is the one exception to "no
...
export default function GhostFibers({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // No WebGL2 support — this is pure decoration, so there's nothing to
      // recover into. The hero's flat background color shows through as-is.
      return;
    }
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uGrain: { value: 0.05 },
        uLightMode: { value: 0 },
        uLineColor: { value: new Float32Array(hexToRgb('#7BE07B')) },
        uGlowColor: { value: new Float32Array(hexToRgb('#FFB6C1')) },
      },
    });
